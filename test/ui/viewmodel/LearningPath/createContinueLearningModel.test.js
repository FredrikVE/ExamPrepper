...
describe("createContinueLearningModel", () => {
	test("presents the active backend module as the stable continue surface", () => {
		...
		expect(createContinueLearningModel({ activeEntry: entry(actionModel), nextActivity: { kind: "start-authored-session" }, t })).toMatchObject({ title: "Continue where you left off", description: "1:Concepts:continue", buttonLabel: "Continue now", actionModel });
	});
	test("keeps the continue surface visible when the read response has no nextActivity", () => {
		const actionModel = { intent: "start", label: "Continue" };
		expect(createContinueLearningModel({ activeEntry: entry(actionModel), nextActivity: null, t })).toMatchObject({ isVisible: true, title: "Continue where you left off", description: "1:Concepts:continue", buttonLabel: "Continue now", actionModel });
	});
