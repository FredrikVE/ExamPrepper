...
describe("createSessionResultModel", () => {
	...
	test("fails fast for an invalid percentage", () => {
		expect(() => model({
			earnedPoints: 4,
			availablePoints: 10,
			percentage: Number.NaN,
			performanceBand: "progress"
		})).toThrow("Invalid learning session percentage: NaN");
	});

	test("fails fast for an unknown backend assessment band", () => {
		expect(() => model({
			earnedPoints: 4,
			availablePoints: 10,
			percentage: 40,
			performanceBand: "unknown"
		})).toThrow("Unknown learning session performance band: unknown");
	});
});
