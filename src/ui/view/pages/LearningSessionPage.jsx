...
export default function LearningSessionPage({ viewModel }) {
	...
	useEffect(() => {
		if (viewModel.currentQuestionRenderKey !== null) questionFocusRef.current?.focus();
		...
		<WorkspaceScaffold className="exam-workspace learning-session-workspace" header={header} footer={null} overlay={overlay} scrollToTopRequestId={viewModel.scrollToTopRequestId}>
			<WorkspaceState state={viewModel.workspaceState}>
				<div className="learning-session-content">
					...
					{viewModel.questionCardModel === null ? null : <LearningSessionStage focusRef={questionFocusRef} focusLabel={viewModel.questionFocusLabel}><QuestionCard key={viewModel.currentQuestionRenderKey} {...viewModel.questionCardModel} /></LearningSessionStage>}
