...
export default function LearningPathStep({ model, onModuleToggle, onStartModule, registerModuleElement }) {
	return (
		<div ref={(element) => registerModuleElement(model.id, element)} className={`learning-path-step learning-path-step--${model.appearance}`} data-learning-path-module-id={model.id}>
			...
			{model.detailModel === null ? null : <LearningPathModuleDetail model={model.detailModel} onStartPressed={() => onStartModule(model.id)} />}
		</div>
	);
}
