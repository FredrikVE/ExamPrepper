...
export default function GlossaryTopicAreaNavigationList({ ariaLabel, allTopicAreaListItem, items }) {
	return (
		<nav className="glossary-topic-area-navigation" aria-label={ariaLabel}>
			<div className="glossary-topic-area-list" role="group" aria-label={ariaLabel}>
				<GlossaryTopicAreaButton item={allTopicAreaListItem} usesOptionSemantics={false} />
				{items.map((item) => (
					<GlossaryTopicAreaButton key={item.topicAreaKey} item={item} usesOptionSemantics={false} />
				))}
			</div>
		</nav>
	);
