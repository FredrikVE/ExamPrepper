...
export default function ProgressDots({ entries, compactEntries, minimalCompactEntries, shouldUseCompactDots, shouldUseResponsiveCompactDots, showEntryOutcome, onSelectEntry, dotsLabel, goToEntryLabel }) {
	...
	return (
		<div className={progressDotsClassName} role="navigation" aria-label={dotsLabel}>
			<div className="progress-pager-dot-list progress-pager-dot-list-normal">
				{entries.map((entry) => (
					<ProgressDot
						...
						showEntryOutcome={showEntryOutcome}
						isCorrect={entry.isCorrect}
						onClick={() => onSelectEntry(entry.entryIndex)}
			...
			<div className="progress-pager-dot-list progress-pager-dot-list-filled-compact">
				{compactEntries.map((entry) => renderCompactProgressEntry({
					entry,
					showEntryOutcome,
					onSelectEntry,
					goToEntryLabel,
			...
			<div className="progress-pager-dot-list progress-pager-dot-list-compact">
				{minimalCompactEntries.map((entry) => renderCompactProgressEntry({
					entry,
					showEntryOutcome,
					onSelectEntry,
					goToEntryLabel,
...
function renderCompactProgressEntry({ entry, showEntryOutcome, onSelectEntry, goToEntryLabel, dotDisplayMode }) {
	if (entry.type === "ellipsis") {
		return (
			<span key={entry.key} className={`progress-pager-dot-ellipsis progress-pager-dot-ellipsis-${dotDisplayMode}`} aria-hidden="true">
				…
			</span>
		);
	}
