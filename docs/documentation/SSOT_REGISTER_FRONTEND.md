...
| `<ProgressBar/>` (renderer) + `buildProgressBarModel()` (presentasjonsavledning) | Lineær fremdrift | `components/Shared/ProgressBar/` | Direkte importører: `MobileDropDownTopBar`, `ExamPage`, `MatchCardsPage`, `LearningSessionPage` |
...
| `MatchCardsGrid` + `MatchCard` | MatchCards-brett og kortinteraksjon | `components/MatchCards/` | Canonical renderer brukt av standalone `MatchCardsPage` og `LearningSessionPage`; eier ikke authored concept-selection eller LearningSession-progresjon |
| `useMatchCardsRoundModel()` | Lokal MatchCards round-state | `viewmodel/MatchCards/` | Canonical round-mekanikk for de samme to konsumentene; eier visuell/interaktiv round-state, mens Page-ViewModels eier overordnet flyt |
