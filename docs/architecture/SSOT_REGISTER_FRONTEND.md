...
Den delte list/detail-shapen og de fire type-pure ressursportene er låst i `docs/architecture/SCOPED_TEST_SET_TRANSPORT_CONTRACT.md` og `test/contracts/scopedTestSetTransportContract.test.js`. `ExamDataSource` eier `/exams`, `ChapterTestDataSource` eier `/chapter-tests`; begge validerer samme eksakte transport-shape og avviser feil scoped `testType` ved boundaryen. Validatoren bruker de eksisterende `TEST_TYPES`- og `LANGUAGES`-eierne og introduserer ingen parallelle enum-registre.
