...
En authored LearningSession starter med én MatchCards-aktivitet før Question-aktivitetene. Backend sender en required `matchCardsTask` med authored glossary-identiteter; `LearningPathRepository` validerer kontrakten og `LearningSessionPageViewModel` eier aktivitetsovergangen `MatchCards → Questions → Result`. Frontend velger ikke pedagogiske concepts lokalt. `Math.random` kan brukes av den delte MatchCards-round-modellen til visuell kortplassering, men ikke til å velge hvilke authored concepts økten inneholder.

Submit sender required `matchCardResults` i backendens authored pair-rekkefølge sammen med Question-svarene. MatchCards completion er dermed del av LearningSession-kontrakten, mens backend fortsatt eier session completion og progression. `QuestionCard` utvides ikke med MatchCards-type eller LearningPath-modusflagg; `MatchCardsGrid` og `useMatchCardsRoundModel()` er en separat canonical capability med standalone MatchCardsPage og LearningSession som konsumenter.
